...
def prepare_data(images, labels, device):
    ...
    labels_onehot = (
        F.one_hot(labels, num_classes=4).permute(0, 3, 1, 2).float().to(device)
    )
    ...
    for epoch in epochs:
        ...
        if verbose > 1:
            if epoch % 10 == 0:
                if valid_dataset is not None:
                    print(
                        f"Epoch [{epoch+1}/{num_epochs}]], Loss: {train_losses[-1]:.4f}, Validation Loss: {valid_losses[-1]:.4f}"
                    )
                else:
                    print(
                        f"Epoch [{epoch+1}/{num_epochs}]], Loss: {train_losses[-1]:.4f}"
                    )
